/*
  ...
  OUTPUT: D the (nxn) or (n2xn1) distance matrix
  cuDist(A,D) 
  cuDist(A,B,D)
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if (init == 0) {
    ...
    // One input
    CUresult status = cuModuleGetFunction(&drvfunf, *drvmod, "gpuPdist");
    if (CUDA_SUCCESS != status) {
      mexErrMsgTxt("Unable to load user function.");
    }
    // Two inputs
    status = cuModuleGetFunction(&drvfunf2, *drvmod, "gpuPdist2");
    if (CUDA_SUCCESS != status) {
      mexErrMsgTxt("Unable to load user function.");
    }
