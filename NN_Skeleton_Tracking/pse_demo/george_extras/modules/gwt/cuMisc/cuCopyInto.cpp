...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if (init == 0) {
    ...
    CUresult status = cuModuleGetFunction(&drvfunf, *drvmod, "kCopyInto");
  ...
  int paddingSize = (int) mxGetScalar(prhs[2]);
  ...
  int imgSize = color ? int(sqrt(sin1[0] / 3)) : int(sqrt(sin1[0]));
  int numImages =  (color ? 3 : 1) * sin1[1]; //image per column
