...
%george
%snapshot_path = ['/misc/BreglerGroup/mayank/3DMotionAnalysis/results/conv_ncar_' cam]; %change this to where weights
snapshot_path = ['results/conv_ncar_' cam]; %change this to where weights
%george
                                        %should be written
snapshotevery = 50; %how often (number of mini-batches) to do snapshots
...
if isunix
  copystring = sprintf('cp ./conv_ncar.m %s',snapshot_path);
  assert(system(copystring)==0);
end

%addpath('/home/gwtaylor/matlab/bregler/greendot/hashing_tinypeople')
fprintf('Loading data.\n');
load([data_path 'init_data_' cam '.mat'],'trainndata','trainlabels_r','trainlabels','validndata','validlabels_r','validlabels');
