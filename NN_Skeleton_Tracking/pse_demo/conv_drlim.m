...
num_connect = 4; 
%DrLim similarity margin
m=1.25;

%Temperature parameter for pose distance
T=1;
...
snapshot_path = '/tmp/conv_drlim'; %change this to where weights
                                        %should be written
...
if isunix
  copystring = sprintf('cp ./conv_drlim.m %s',snapshot_path);
...
numlabels = size(trainlabels_r,1); %use relative labels
...
fprintf(['Mini-batch number; Training cost (Soft DRLIM objective); Validation ' ...
         'cost (Soft DRLIM objective); Validation cost (Pixel error)\n']);
for bb=1:numbatches %process this many random mini-batches
  ...
  vv = [filters1(:);b1(:);convcoeff1(:);filters2(:);b2(:);convcoeff2(:);A(:)];
    ...
    p = normalize_pose_distance(trainlabels_r(:,batchrndidx),T);
  ...
  [X,fX,i]=minimize(vv,'convnetdrlim',maxlinesearches,Dim, ...
      data(:,batchrndidx),p,connections,m, ...
  ...
  if mod(bb,snapshotevery)==0
      ...
      [err_train,err_trainmax,err_valid,err_validmax,err_pix] = convnetdrlimeval_gpu(X,Dim,data,trainlabels,vd,validlabels,m,connections,useabsrec,skiphead);
      % these are the only meaningful metrics
      ...
      %save snapshot
      snapshot_file=sprintf('%s/conv_drlim_snapshot_batch%d.mat',snapshot_path,bb);
