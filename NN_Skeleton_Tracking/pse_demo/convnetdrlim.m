function [f, df] = convnetdrlim(VV,Dim,x,p, ...
  connections,m,weightdecay,useabsrec)

  % The main function for the convnet (Soft DrLIM objective)
  % It first does a forward pass through the convnet and projects down to
  % the low-dimensional space
  % It then computes the Soft DrLIM cost, f
  % 
...
[yy,sigmap1,map1,y1,sigmap2,map2] = convnet_forward2_gpu(x_d,filters1_d,b1_d, ...
  ...
  md = m-n1; %on CPU
  idx = find(md<0); %find points outside margin
  idx = [idx' 1:numcases+1:numcases*numcases]; %also include points on diagonal
  md(idx)=0;
  ...
  p_d = GPUsingle(p); %to GPU  
  np_d = 1-p_d;  
  %Similarity gradient
  dfsdz = zeros(numcases,numhid,GPUsingle);
    
  %bsxfun version of the above
  b = p_d+p_d';  
  cuNCAreg(z,b,dfsdz);
  dfsdz=transpose(dfsdz); %due to cuNCAreg function
   
  %Dissimilarity gradient (GPU)
  %Note there may be wasted computation here
  %Perhaps better to just loop through elements where n<m
  dfddz = zeros(numcases,numhid,GPUsingle);
  
  a = n<m; %1 where norm falls within margin, 0 everywhere else
  assign(1,a,GPUsingle(0),[1,numcases+1,END]); %zero on diagonal
  b = np_d+np_d';
  GPUtimes(b,n-m,b);
  GPUrdivide(b,n+eps,b); %in case n is zero
  GPUtimes(b,a,b); %n<m and diagonals are zero
    
  cuNCAreg(z,b,dfddz);
  dfddz=transpose(dfddz); %due to cuNCAreg function

  clear z d n p_d np_d %free up some memory
  
  %gradient of f with respect to the input of the code layer
  %dfdz = -(2*lambda/numcases)*dfdz;
  dfdz = (dfsdz+dfddz)/numcases; %combine
  dfdA = dfdz*yy';
