...
spmaps = [1 size(maps,2)];
% Pooled indices.
% pinds = ones(spmaps,GPUsigle);
...
px = 5;
py = 5;
numelMaps = size(maps,1);
mapPtr = (getPtr(maps));
GPUcompileStart('gpumax','-f',maps,pmaps,mapPtr,numelMaps)
...
ind = cublasIsamax(numelMaps,mapPtr+jt*numelMaps);
%                 assign(1,pmaps,slice(maps,linind,jt,map,image),it,jt,map,image);
assign(1,pmaps,slice(maps,ind,jt),1,jt);
%             assign
% GPUend
% GPUend
            GPUend
