 ...
 * @data Aug 14, 2010
 *
 * @pooltoolbox_file @copybrief reverse_max_pool3d.cpp
 *
 *****************
 * How to compile:
 *
...
/*!
 * @copybrief reverse_max_pool3d.cpp
 *
        ...
        
{
    ...
    int *number_threadsp;
    int blocksx, blocksy, blocksk, remx, remy, remk, bkjump, bcoljump, plane3jump, plane4jump, top_left;
    int poolx, pooly, poolk, poolsizex, poolsizey, poolsizek;
    const mwSize *unpooldims;
...
//     mexPrintf("\n\npooldims: %d %f %p\n\n",pooldims,pooldims,pooldims); 
    if((int)pooldims[1]==2){ // 2D Pooling
        mexErrMsgTxt("There must be 3 pooling sizes (need a third dimension).");        
    }else{
    ...
    if(poolx*pooly*poolk>65536){
        mexErrMsgTxt("Cannot pool in 3D larger than 65536 total pool region size due to uint16 indices limitation.");
    }
    ...
    // Number of blocks in k dimension
           if(num_dims<3){ // For case where there is only one pooled maps na dimage.
            num_dims = 3;
            blocksk = 1;
        }else{
        blocksk = (int)dims[2];
    ...
    outputdims = (int*) mxCalloc(num_dims, sizeof(int));
    ...
    if(nrhs < 4 || mxIsEmpty(UNPOOLED_SIZE)==true){ // unpooled_size wasn't passed in, so create it
        ...
        outputdims[2] = (int)poolp[2]*dims[2];
        ...
        unpooldims = mxGetDimensions(UNPOOLED_SIZE);
//         mexPrintf("Unpooled size: %f x %f x %f, %d\n",unpooled_sizep[0], unpooled_sizep[1], unpooled_sizep[2],unpooldims);
        if((int)unpooldims[1]==2){ // 2D Pooling
            mexErrMsgTxt("There must be 3 unpooled sizes (need a third dimension).");
        }
        outputdims[0] = (int)unpooled_sizep[0];
        outputdims[1] = (int)unpooled_sizep[1];
        outputdims[2] = (int)unpooled_sizep[2];
        // Remaining dimensions are same as input.
        for(int k=3;k<(int)num_dims;k++){
            outputdims[k] = dims[k];
        }
...
//     mexPrintf("Number of output dimensions: %d, %d %d %d %d",num_dims,outputdims[0],outputdims[1],outputdims[2],outputdims[3]);
...
//                     mexPrintf("Input Index: %d, i: %d, j: %dm top_left: %d, outputelem: %d, dims[0]: %d\n",maxi+1,i,j,top_left,outputelem,dims[0]);
        ...
        int brow, bcol, bk, i, j, k, outputelem, plane3 = 0;
        unsigned short int maxi=0;
        #pragma omp parallel for num_threads(number_threads) shared(imagesp, outputp, indicesp) private(plane3, brow, bcol, bk, i, j, k, top_left, maxi, outputelem)
        ...
        for(bk=0;bk<blocksk;bk++){
            for(bcol=0;bcol<blocksy;bcol++){ // Loop over blocks in columns
                for(brow=0;brow<blocksx;brow++){ // Loop of blocks in rows
                    ...
                    // Have to skip (brow*poolx)+i elements down, bcol*poolx+j elements over.
                    outputelem = (bk*poolk+k)*(outputdims[0]*outputdims[1])+brow*poolx+i+(bcol*pooly+j)*outputdims[0];
//                     mexPrintf("Input Index: %d, i: %d, j: %dm top_left: %d, inputelem: %d, dims[0]: %d\n",maxi+1,i,j,top_left,inputelem,dims[0]);
        ...
        int brow, bcol, bk, i, j, k, outputelem, plane3, plane4 = 0;
        unsigned short int maxi=0;
        #pragma omp parallel for num_threads(number_threads) shared(imagesp, outputp, indicesp) private(plane4, plane3, brow, bcol, bk, i, j, k, top_left, maxi, outputelem)
            ...
            for(bk=0;bk<blocksk;bk++){            
                for(bcol=0;bcol<blocksy;bcol++){ // Loop over blocks in columns
                    for(brow=0;brow<blocksx;brow++){ // Loop of blocks in rows
                        ...
                        // Have to skip (brow*poolx)+i elements down, bcol*poolx+j elements over.
                        outputelem = plane4*(outputdims[0]*outputdims[1]*outputdims[2])+(bk*poolk+k)*(outputdims[0]*outputdims[1])+brow*poolx+i+(bcol*pooly+j)*outputdims[0];
