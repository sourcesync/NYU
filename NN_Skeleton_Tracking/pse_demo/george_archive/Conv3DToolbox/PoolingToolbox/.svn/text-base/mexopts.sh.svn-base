...
# note that cce is the 64-bit version and cc is the 32-bit version
            RPATH="-Wl,-rpath-link,$TMW_ROOT/bin/$Arch"
            CC='/opt/intel/cce/current/bin/intel64/icc'
            CFLAGS='-ansi -D_GNU_SOURCE -fexceptions'
            ...
            CDEBUGFLAGS='-g'
#
            CXX='/opt/intel/cce/current/bin/intel64/icc'
...
#
            LD="$COMPILER"
            LDEXTENSION='.mexa64'
            LDFLAGS="-pthread -shared -Wl,--version-script,$TMW_ROOT/extern/lib/$Arch/$MAPFILE -Wl,--no-undefined -openmp"
            LDOPTIMFLAGS='-O3'
            LDDEBUGFLAGS='-g'
#
            LDCXX="$COMPILER"
            LDCXXEXTENSION='.mexa64'
            LDCXXFLAGS="-pthread -shared -Wl,--version-script,$TMW_ROOT/extern/lib/$Arch/$MAPFILE -Wl,--no-undefined -openmp"
            LDCXXOPTIMFLAGS='-O3'
            LDCXXDEBUGFLAGS='-g'
#
            POSTLINK_CMDS=':' 
