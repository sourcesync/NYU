 ...
 * @data Feb 11, 2010
 *
 * @pooltoolbox_file @copybrief max_pool3d.cpp
 *
 *****************
 * How to compile:
 *
...
#define POOL_SIZE       prhs[1] // The size of the subregions to pool over: 1x3  matrix indicating the p x q x k size of the region.
        ...
        
{
    ...
    int *number_threadsp;
    int blocksx, blocksy, blocksk, remx, remy, remk;
    int bcoljump, bkjump, plane3jump, plane4jump, top_left;
    int poolx, pooly, poolk, poolsizex, poolsizey, poolsizek;
    
    const mwSize *dims;
    const mwSize *inddims;
    const mwSize *pooldims;
    int* outputdims = NULL;
...
//      }
    ...
    poolx = (int) poolp[0];
    pooly = (int) poolp[1];
    pooldims = mxGetDimensions(POOL_SIZE);
    if((int)pooldims[1]==2){ // 2D Pooling
//         mexPrintf("Pool size passed in was: %f x %f x %f, numpooldims: %d\n",poolp[0],poolp[1],poolp[2],(int)pooldims);
        mexErrMsgTxt("max_pool3d.mex There must be 3 pooling sizes (need a third dimension).");        
    }
    poolk = (int) poolp[2];
        
    
    if(poolx*pooly*poolk>65536){
        mexErrMsgTxt("Cannot pool in prod(pool_size) larger than 65536 total pool region size due to uint16 indices limitation.");
    }
    ...
    blocksk = (int)dims[2]/poolk;
    // Remaining blocks bast blocksk
    remk = (int)(dims[2])%poolk;
    
    // If there are remaining blocks, we have to do those partial blocks.
    if(remx>0)
        blocksx = blocksx+1;
    if(remy>0)
        blocksy = blocksy+1;
...
//     mexPrintf("blocksx: %d blocksy: %d,   remx: %d, remy %d\n", blocksx, blocksy, remx, remy);
    ...
    outputdims[2] = blocksk;
    // Remaining dimensions are same as input.
    for(int k=3;k<(int)num_dims;k++){
        outputdims[k] = dims[k];
    }
    ...
    bkjump=dims[0]*dims[1]*poolk; // How big to jump each blockk
...
//     mexPrintf("Number of output dimensions: %d %d %d %d, bcoljump %d plane3jump %d",outputdims[0],outputdims[1],outputdims[2],outputdims[3],bcoljump,plane3jump);
    ...
    if(INDICES_PROVIDED==false){ // Have to actually do the max pooling
        if(num_dims==2){ // Just pool the single image (ignore the 3D pooling). 
...
//             mexPrintf("dims: %d x %d x %d \n", dims[0], dims[1], dims[2]);
            float max, min = 0;
            int brow, bcol, bk, i, j, k, plane3 = 0;
            unsigned short int maxi, mini = 0;
            
            #pragma omp parallel for num_threads(number_threads) shared(imagesp, outputp, indicesp) private(plane3, brow, bcol, bk, i, j, k, top_left, poolsizex, poolsizey, poolsizek, max, min, maxi, mini)
            for(bk=0;bk<blocksk;bk++){    
                for(bcol=0;bcol<blocksy;bcol++){ // Loop over blocks in columns
                    for(brow=0;brow<blocksx;brow++){ // Loop of blocks in rows
                        ...
                        if(remx>0 && brow==blocksx-1){poolsizex = remx;}else{poolsizex = poolx;}
                        if(remy>0 && bcol==blocksy-1){poolsizey = remy;}else{poolsizey = pooly;}
                        if(remk>0 && bk==blocksk-1){poolsizek = remk;}else{poolsizek = poolk;}                                                
                        
                        // Linear index into the top left element of the block (note this uses the original block sizes always).
                        top_left = bk*(bkjump)+bcol*(bcoljump)+brow*poolx;
...
//                     mexPrintf("poosizex: %d poolsizey: %d topleft: %d, brow %d, bcol %d\n",poolsizex,poolsizey,top_left,brow,bcol);
                        ...
                        maxi = 0;
                        mini = 0;
                        for(k=0;k<poolsizek;k++){
                            for(j=0;j<poolsizey;j++){
                                for(i=0;i<poolsizex;i++){ // Loop over rows of pool region
                                    int elem = top_left+k*plane3jump+j*dims[0]+i;
                                    if(imagesp[elem]>max){
                                        max = imagesp[elem];
                                        maxi = k*poolx*pooly+j*poolx+i;
                                    }else if(imagesp[elem]<min){
                                        min = imagesp[elem];
                                        mini = k*poolx*pooly+j*poolx+i;
                                    }
//                             mexPrintf("%f (%d) i,j(%d,%d) (max,min)=(%f,%f) (maxi,mini)=(%d,%d)\n", imagesp[top_left+j*dims[0]+i],top_left+j*dims[0]+i,i,j,max,min,maxi,mini);
                                }
                            }
...
//                         mexPrintf("Index: %d\n",plane3*(blocksx*blocksy)+bcol*blocksx+brow);
                        int outputelem = bk*(blocksx*blocksy)+bcol*blocksx+brow;
...
//             mexPrintf("dims: %d x %d x %d \n", dims[0], dims[1], dims[2]);
            float max, min = 0;
            int brow, bcol, bk,i, j, k, plane3, plane4 = 0;
            unsigned short int maxi, mini = 0;
            
            #pragma omp parallel for num_threads(number_threads) shared(imagesp, outputp, indicesp) private(plane4, plane3, brow, bcol, bk, i, j, k, top_left, poolsizex, poolsizey, poolsizek, max, min, maxi, mini)
                ...
                for(bk=0;bk<blocksk;bk++){
                    for(bcol=0;bcol<blocksy;bcol++){ // Loop over blocks in columns
                        for(brow=0;brow<blocksx;brow++){ // Loop of blocks in rows
                            ...
                            // Linear index into the top left element of the block (note this uses the original block sizes always).
                            top_left = plane4*(plane4jump)+bk*(bkjump)+bcol*(bcoljump)+brow*poolx;
...
//                     mexPrintf("Max: %f Index: %d\n\n",max,maxi);
//                         mexPrintf("Index: %d\n",plane3*(blocksx*blocksy)+bcol*blocksx+brow);
                            int outputelem = plane4*(blocksx*blocksy*outputdims[2])+bk*(blocksx*blocksy)+bcol*blocksx+brow;
                            outputp[outputelem] = max;
                            indicesp[outputelem] = maxi;
                        }
                    }
...
//                     mexPrintf("Input Index: %d, i: %d, j: %dm top_left: %d, inputelem: %d, dims[0]: %d\n",maxi+1,i,j,top_left,inputelem,dims[0]);                                        
                    ...
                    outputp[top_left] = imagesp[inputelem];
                    indicesp[top_left] = inputindsp[top_left];
                }
            }
        }else if(num_dims==3){ // Pool each plane independently
            int brow, bcol, bk, i, j, k, inputelem, plane3 = 0;
            float max;
            unsigned short int maxi=0;
            
            #pragma omp parallel for num_threads(number_threads) shared(imagesp, outputp, indicesp, inputindsp) private(plane3, brow, bcol, bk, i, j, k, top_left, maxi, max, inputelem)
                for(bk=0;bk<blocksk;bk++){
                for(bcol=0;bcol<blocksy;bcol++){ // Loop over blocks in columns
                    for(brow=0;brow<blocksx;brow++){ // Loop of blocks in rows
                        
                        // Linear index into the top left element of the block (note this uses the original block sizes always).
                        top_left = bk*(blocksx*blocksy)+bcol*(blocksx)+brow;
                        
                        // Get the index at the pooled location from the input indices.
                        maxi = inputindsp[top_left];
                        // Get the row and column offsets.
                        k = (int)(maxi/(poolx*pooly)); // Get the k index first.
                        maxi = (maxi%(poolx*pooly)); // Now this is the normal spatial index.
                        i = maxi%poolx;
                        j = (int)(maxi/poolx);
                        
                        // Have to skip (brow*poolx)+i elements down, bcol*poolx+j elements over.
                        inputelem = (bk*poolk+k)*(dims[0]*dims[1])+brow*poolx+i+(bcol*pooly+j)*dims[0];
//                     mexPrintf("Input Index: %d, i: %d, j: %dm top_left: %d, inputelem: %d, dims[0]: %d\n",maxi+1,i,j,top_left,inputelem,dims[0]);
            ...
            int brow, bcol, bk, i, j, k, inputelem, plane3, plane4 = 0;
            float max;
            unsigned short int maxi=0;
            #pragma omp parallel for num_threads(number_threads) shared(imagesp, outputp, indicesp, inputindsp) private(plane4, plane3, brow, bcol, bk, i, j, k, top_left, maxi, max, inputelem)
            for(plane4=0;plane4<outputdims[3];plane4++){
                for(bk=0;bk<blocksk;bk++){
                    for(bcol=0;bcol<blocksy;bcol++){ // Loop over blocks in columns
                        for(brow=0;brow<blocksx;brow++){ // Loop of blocks in rows
                            ...
                            // Linear index into the top left element of the block (note this uses the original block sizes always).
                            top_left = plane4*(blocksx*blocksy*blocksk)+bk*(blocksx*blocksy)+bcol*(blocksx)+brow;
                            ...
                            // Have to skip (brow*poolx)+i elements down, bcol*poolx+j elements over.
                            inputelem = plane4*(dims[0]*dims[1]*dims[2])+(bk*poolk+k)*(dims[0]*dims[1])+brow*poolx+i+(bcol*pooly+j)*dims[0];
//                     mexPrintf("Input Index: %d, i: %d, j: %dm top_left: %d, inputelem: %d, dims[0]: %d\n",maxi+1,i,j,top_left,inputelem,dims[0]);
