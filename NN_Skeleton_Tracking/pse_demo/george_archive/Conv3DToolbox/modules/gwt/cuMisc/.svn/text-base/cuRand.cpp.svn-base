...
void hostRandomUniform(CUfunction drvfun, int N, int nrhs, hostdrv_pars_t *prhs) {
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if (init == 0) {
    ...
    status = cuModuleGetFunction(&drvfunf, *drvmod, "kRandomUniform");
    if (CUDA_SUCCESS != status) {
      mexErrMsgTxt("Unable to load user function.");
    }
  ...
  hostRandomUniform(drvfun, N, gpunrhs, gpuprhs);
}
