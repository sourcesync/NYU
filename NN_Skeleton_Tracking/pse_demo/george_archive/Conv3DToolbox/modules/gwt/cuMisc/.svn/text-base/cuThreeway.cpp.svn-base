...
void hostGPUmisc(int nrhs, hostdrv_pars_t *prhs, int numA, int numB, int numC, int numCases) {
  ...
  CUresult err = CUDA_SUCCESS;
    ...
    err = cuLaunchGridAsync(drvfunf, NUM_APPLY_BLOCKS, 1, 0);
    if (CUDA_SUCCESS != err) {
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  int gpunrhs = 4;
  gpuprhs[0] = hostdrv_pars(&d_A,sizeof(d_A));
  gpuprhs[1] = hostdrv_pars(&d_B,sizeof(d_B));
  gpuprhs[2] = hostdrv_pars(&d_C,sizeof(d_C));
  gpuprhs[3] = hostdrv_pars(&d_OUT,sizeof(d_OUT));
