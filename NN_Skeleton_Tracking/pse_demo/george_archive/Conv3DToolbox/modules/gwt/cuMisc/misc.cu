...
extern "C" {
  ...
  */
  __global__ void THREEWAYPROD(int N, int numA, int numB, int numC, int numCases, float *A, float *B, float *C, float *target) {
    ...
    //target index
    unsigned int xIndex = blockIdx.x * NUM_APPLY_THREADS_PER_BLOCK + threadIdx.x;

    for (unsigned int i=xIndex; i < N; i+= blockDim.x * gridDim.x) {
      //indices to inputs
      const unsigned int a_idx = i % numA;
      const unsigned int b_idx = ( i % (numA*numB) ) / numA;
      const unsigned int c_idx = i / (numA*numB);
