/*!
 * @brief GPUmat GPU implementation of the matlab min function (see implementation
 * for min).
 * 
 * @file
 ...
 * @data Apr 29, 2011
 *
 * @pooltoolbox_file @copybrief min.cpp
 * @gpu_file @copybrief min.cpp
 *
 *****************
 * How to compile:
 *
 ...
 * >> make all
 *
 * Installs in GPUmat's @GPUtype folder so you can call min directly. 
 */
#include <stdio.h>
#include <string.h>
#include <stdarg.h>
 ...
 * @copybrief min.cpp
 *
 * @param INPUT   	      prhs[0] // The input you want min of. Does min along first non-singleton if no other parameters are passed in.
 * @param INPUT2          prhs[1] // The same size as input or a scalara to compare to. Pass [] to do max over dimensions.
 * @param DIM             prhs[2] // If [] is arg 2 then this is the dimension to take min over.
 *
 * @retval MAX   	      plhs[0] // Corresponding mins.
 * @retval IND            plhs[1] // Locations of the mins along the dimension specified (only when specifying a dimension).
 */
...
//         CUresult status = cuModuleGetFunction(&drvfunf, *drvmod, "MAXPOOLF");
        CUresult status = cuModuleGetFunction(&drvfunMatrices, *drvmod,  "MINSAMESIZE");
        if (CUDA_SUCCESS != status) {
            mexErrMsgTxt("Unable to load user function.");
        }
        status = cuModuleGetFunction(&drvfunScalar, *drvmod,  "MINSCALAR");
        if (CUDA_SUCCESS != status) {
            mexErrMsgTxt("Unable to load user function.");
        }
        status = cuModuleGetFunction(&drvfunDim1, *drvmod,  "MINDIM1");
        if (CUDA_SUCCESS != status) {
            mexErrMsgTxt("Unable to load user function.");
        }
        status = cuModuleGetFunction(&drvfunDimLast, *drvmod,  "MINDIMLAST");
        if (CUDA_SUCCESS != status) {
            mexErrMsgTxt("Unable to load user function.");
        }
        status = cuModuleGetFunction(&drvfunDimMiddle, *drvmod,  "MINDIMMIDDLE");
        if (CUDA_SUCCESS != status) {
            mexErrMsgTxt("Unable to load user function.");
        }
        init = 1;
