 ...
 * @data Apr 14, 2011
 *
 * @pooltoolbox_file @copybrief cuRevMaxPool.cpp
 * @gpu_file @copybrief cuRevMaxPool.cpp
 *
 *****************
 * How to compile:
 *
 ...
 * @param UNPOOLED_SIZE prhs[3] // The unpooled size of the images which is typically n*p x m*q (just cares about the first two dimensions so only input those as a 1x2 matrix).
...
void hostDriver(CUfunction drvfun, int N, int nrhs, hostdrv_pars_t *prhs, int imx, int imy, int outx, int ouy, int poolx, int pooly);
...
//         CUresult status = cuModuleGetFunction(&drvfunf, *drvmod, "MAXPOOLF");
        CUresult status = cuModuleGetFunction(&drvfunfunp, *drvmod,  "REVAVGPOOLF");
...
//     mexPrintf("nrhs %d\n",nrhs);
    ...
    if(pdims[1]<2){
        ...
        mexErrMsgTxt("The pooling sizes must be an array of atleast 2 dimensions for 2D pooling.");
    }
    ...
    if(nrhs>=4 || mxIsEmpty(prhs[3])==true){
        ...
        numunpdims = 2;
        unpdims = (int*) mxCalloc(numunpdims,sizeof(int));          
        unpdims[1] = 2; // Just 2D pooling.s
        unpsize = (double*) mxCalloc(2, sizeof(double));

        unpsize[0] = (double) im_size[0]*psize[0];
        unpsize[1] = (double) im_size[1]*psize[1];
    }
    // Make sure only use up to the planes dimension. 
    if(unpdims[1]>3){
        unpdims[1] = 3;
    }
