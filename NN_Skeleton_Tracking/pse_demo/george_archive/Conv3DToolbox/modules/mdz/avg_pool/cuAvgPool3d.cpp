...
//         CUresult status = cuModuleGetFunction(&drvfunf, *drvmod, "MAXPOOLF");
        CUresult status = cuModuleGetFunction(&drvfunfnoind, *drvmod,  "AVGPOOLFNOIND3D");
        if (CUDA_SUCCESS != status) {
            mexErrMsgTxt("Unable to load user function.");
        }
        
//         CUresult status = cuModuleGetFunction(&drvfunfind, *drvmod,  "AVGPOOLFIND3D");
//         if (CUDA_SUCCESS != status) {
//             mexErrMsgTxt("Unable to load user function.");
//         }
        init = 1;
    ...
    int pnumel = mxGetNumberOfElements(prhs[1]);
    if(pnumel<3){
        mexPrintf("Number of pooling size elements: %d\n",pnumel);
