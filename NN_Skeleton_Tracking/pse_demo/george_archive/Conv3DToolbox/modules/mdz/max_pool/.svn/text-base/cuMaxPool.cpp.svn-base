 ...
 * @data Apr 20, 2011
 *
 * @pooltoolbox_file @copybrief cuMaxPool.cpp
 * @gpu_file @copybrief cuMaxPool.cpp
 *
 *****************
 * How to compile:
 *
...
//         CUresult status = cuModuleGetFunction(&drvfunf, *drvmod, "MAXPOOLF");
        CUresult status = cuModuleGetFunction(&drvfunfnoind, *drvmod,  "MAXPOOLFNOIND");
        if (CUDA_SUCCESS != status) {
            mexErrMsgTxt("Unable to load user function.");
        }
        
        status = cuModuleGetFunction(&drvfunfind, *drvmod,  "MAXPOOLFIND");
        if (CUDA_SUCCESS != status) {
            mexErrMsgTxt("Unable to load user function.");
        }
        init = 1;
