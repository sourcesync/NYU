...
#----------------------------------------------------------------------------
            RPATH="-Wl,-rpath-link,$TMW_ROOT/bin/$Arch"
            CC='/opt/intel/Compiler/11.1/080/bin/ia32/icc'
            CFLAGS='-ansi -D_GNU_SOURCE -fexceptions'
            CFLAGS="$CFLAGS -fPIC -fno-omit-frame-pointer -openmp -pthread"
            CLIBS="$RPATH $MLIBS -lm"
            COPTIMFLAGS='-O -DNDEBUG'
            CDEBUGFLAGS='-g'
#
            CXX='/opt/intel/Compiler/11.1/080/ia32/bin/icc'
# Ensure we have OpenMP in here too
            CXXFLAGS='-ansi -D_GNU_SOURCE -openmp'
            CXXFLAGS="$CXXFLAGS -fPIC -fno-omit-frame-pointer -pthread"
            CXXLIBS="$RPATH $MLIBS -lm"
...
#----------------------------------------------------------------------------
            RPATH="-Wl,-rpath-link,$TMW_ROOT/bin/$Arch"
            CC='/opt/intel/Compiler/11.1/080/bin/intel64/icc'
#            CFLAGS='-fno-common -no-cpp-precomp -fexceptions'
            CFLAGS='-ansi -D_GNU_SOURCE -fexceptions'
            CFLAGS="$CFLAGS -fPIC -fno-omit-frame-pointer -openmp -pthread"
#            CLIBS="$RPATH $MLIBS -lm"
            ...
            CDEBUGFLAGS='-g'
#
            CXX='/opt/intel/Compiler/11.1/080/bin/intel64/icc'
# Ensure we have OpenMP in here too
            CXXFLAGS='-ansi -D_GNU_SOURCE -openmp'
            CXXFLAGS="$CXXFLAGS -fPIC -fno-omit-frame-pointer -pthread"
