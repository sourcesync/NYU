/*!
 * @brief MEX wrapper for IPP and OpenMP implementation of the valid_each3_sum4 operation.
 ...
 * @data Mar 11, 2010
 *
 * @conv_file @copybrief valid_each3_sum4.cpp
 ...
 * >> mex -f ./Helper/ipp/mexopts.sh -I/opt/intel/ipp/current/em64t/include -L/opt/intel/ipp/current/em64t/lib  -L/opt/intel/cce/current/lib -lguide  -lippiemergedem64t -lippimergedem64t  -lippcoreem64t  -lippsemergedem64t   -lippsmergedem64t  -lstdc++ valid_each3_sum4.cpp
 *
 * Normal output:<br>
 * valid_each3_sum4.cpp(229): (col. 1) remark: OpenMP DEFINED LOOP WAS PARALLELIZED.<br>
 * >>
 *
...
// Input Arguments
#define	MAPS   	        prhs[0] // The stack of feature maps you want to convolve: n x m x k x num_images matrix of single precision floating point numbers.
...
//     mexPrintf("%d images at %d by %d\n", num_image_maps, image_size.width, image_size.height);
//     mexPrintf("%d x %d kernels at %d by %d\n", num_kernels,kerneldims[3], kernel_size.width, kernel_size.height);
...
//     // Can't modify kernel directly to multiply by C(j,k).(k*num_input_maps*kernel_size.width*kernel_size.height)
//     int tempkerndims[4];
...
//         
//         #pragma omp parallel for num_threads(number_threads) shared(j) private(k, kernelp, conmatp, temp_kernelp, kernelValue, conmatValue) schedule(static,1)        
...
//#pragma omp parallel for num_threads(number_threads) shared(imagep_base, kernelp_base, outputp_base, output_size, kernel_size, image_size, outputdims) private(i, imagep, kernelp, outputp)
    ...
    if(num_images==1){
        ...
        for (j=0;j<num_input_maps;j++){
            ...
            #pragma omp parallel for num_threads(number_threads) shared(j) private(k, indi, indj, imagep, kernelp, outputp, conmatp)
            for(k=0;k<num_feature_maps;k++){
                // Update the pointer in the connectivity matrix.
                ...
                if(*conmatp != 0){
                    ...
                    imagep = imagep_base + (k*image_size.width*image_size.height);
                    // set pointer offset for kernel
                    kernelp = kernelp_base + (j*kernel_size.width*kernel_size.height) + (k*num_input_maps*kernel_size.width*kernel_size.height);
        ...
        #pragma omp parallel for num_threads(number_threads) private(im, j, k, indi, indj, imagep, kernelp, outputp, conmatp)
            ...
            for (j=0;j<num_input_maps;j++){        
                for(k=0;k<num_feature_maps;k++){
                    ...
                    if(*conmatp != 0){
                        ...
                        // set pointer offset for kernel
                        kernelp = kernelp_base + (j*kernel_size.width*kernel_size.height) + (k*num_input_maps*kernel_size.width*kernel_size.height) + (im*num_feature_maps*num_input_maps*kernel_size.width*kernel_size.height);
                        // set pointer offset for output in it's 4D array.
