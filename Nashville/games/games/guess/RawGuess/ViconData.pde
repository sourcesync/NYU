...
class ViconData {
  ...
  ViconData(boolean simulated, boolean slave) { 

    this.simulated = simulated;

    minXYZ = new PVector(-4000, -12000, -2000);
    maxXYZ = new PVector(4000, 12000, 2000);
    ...
    if (simulated) {
      ...
      if (!slave) {
        String logname = "log_" + year() + month() + day() + hour() + minute() + second();
        logfile = createWriter(logname + ".txt"); 
      }
    }
  }
  ...
  PVector normalize(PVector p) {
   PVector ret = PVector.sub(p,minXYZ);
   ret.div(deltaXYZ);
   return ret;
  }
  ...
  String vicon_proxy_get_data() {
    try {
      ...
      if ( pa.getLength()!=0 ) {
        ...
        if (logfile != null && message.substring(0,1).equals("{")) {
