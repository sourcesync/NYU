...
import json
import subprocess
from subprocess import Popen # spawn system command
...
parser.add_option("-o", "--object", action="store", type="string",
                  dest="vicon_object", default="calibwand", help="The Vicon object as defined in Tracker (default='calibwand')")
parser.add_option("-x", "--x-axis", action="store", type="int", dest="x", default=1, help="Vicon channel for x axis, as displayed on screen (default=1)")
parser.add_option("-y", "--y-axis", action="store", type="int", dest="y", default=2,help="Vicon channel for y axis, as displayed on screen (default=2)")
parser.add_option("-z", "--z-axis", action="store", type="int", dest="z", default=3,help="Vicon channel for z axis, as displayed on screen (default=3)")
parser.add_option("-g", "--ignore-vicon", action="store_true", dest="ignore_vicon", default=False, help="Ignore Vicon data and just use mouse (default=False)")
parser.add_option("-s", "--simulation", action="store_true", dest="simulation_mode", default=False,help="Simulates Vicon data using vicon.txt file (default=False)")
parser.add_option("-l", "--sleep", action="store", type="int", dest="sleep", default=3,help="Time to sleep upon reset, in seconds, to get the ball in place (default=3)")

parser.add_option("-X", "--x-window", action="store", type="float", dest="xwin", default=2000, help="Room rectangle:x,mm (default=2000)")
parser.add_option("-Y", "--y-window", action="store", type="float", dest="ywin", default=2000, help="Room rectangle:y,mm (default=2000)")
parser.add_option("-q", "--disable-zvel", action="store_true", dest="disablezvel", default=False, help="Disable z axis velocity control (default=False)")
parser.add_option("-e", "--pyspell", action="store_true", dest="pyspell", default=False, help="Launch PySpell as subprocess (default=False)")
parser.add_option("--twitter", action="store_true", dest="twitter", default=False, help="Enable twitter (default=False)")
parser.add_option("-V", "--orig-velocity", action="store", type="float", dest="origvelocity", default=1.01, help="Original velocity (default=1.01)")
...
class HelloWorld:
    ...
    def restart(self):
        if not options.ignore_vicon:
            obj = []
            if options.simulation_mode: #read from file
                l = self.f.readline()
                if l:
                    obj = json.loads(l) # JSON load
                else:
                    self.f.close()
                    raise "Could not load object from file"
            else: # read from Vicon

                while not obj:
                    print "Waiting for Vicon"
                    obj = self.vp.get_json()

            #get object of interest
            objs = obj['objs']

            #Fancy way to return index to the vicon object
            index = next((i for i in xrange(len(objs)) if objs[i]['name'] == options.vicon_object), None)    

            # more informative error message
            try: assert(index is not None) # must exist here
            except AssertionError:
                print "Could not find %s in the Vicon stream: check command line option -o" % options.vicon_object
                raise

            if index is not None:
                t = objs[index]['t']
                if self.xr:
                    self.viconmousepos[0]=-t[self.idx[0]]
                else:
                    self.viconmousepos[0]=t[self.idx[0]]
                if self.yr:
                    self.viconmousepos[1]=-t[self.idx[1]]
                else:
                    self.viconmousepos[1]=t[self.idx[1]]
                if self.zr:
                    self.viconmousepos[2]=-t[self.idx[2]]
                else:
                    self.viconmousepos[2]=t[self.idx[2]]

            print self.viconmousepos

            # create a volume around the current location of the object
            # this is used to map Vicon co-ordinates to room co-ordinates
            # self.roomrect = Rectangle(self.viconmousepos[0],self.viconmousepos[1],1000,1000,1)

            activevolume=(options.xwin,options.ywin) # mm volume around person which is mapped to drawing region
            self.roomrect = Rectangle(self.viconmousepos[0]-activevolume[0]/2.0,self.viconmousepos[1]-activevolume[1]/2.0,activevolume[0],activevolume[1],1)

            self.zstart = self.viconmousepos[2] #baseline - this is default velocity
        ...
        self.stop = False
        self.maxrlevel = 2
        self.origvelocity = options.origvelocity
...
#             return spoint * 1.5 + self.scenter
    ...
    def timertick(self):
        ...
        if options.ignore_vicon:
            spoint = self.getrawpointer()
        else:
            
            self.update_viconmousepos() # update vicon object pos
            spoint = self.get_mouse_coordinates() # convert to mouse co-ordinates

            if not options.disablezvel:
                self.set_velocity()


        # check to see if mouse pointer is in reset box
        h=512
        if spoint.x>self.reset_offset[0] and spoint.x<(self.reset_offset[0]+self.reset_size[0]) and spoint.y>(h-(self.reset_offset[1]+self.reset_size[1])) and spoint.y<(h-self.reset_offset[1]):
            print "reset box!"
        ...
        if options.twitter:
            # check to see if mouse pointer is in twitter box
            if spoint.x>self.tweet_offset[0] and spoint.x<(self.tweet_offset[0]+self.tweet_size[0]) and spoint.y>(h-(self.tweet_offset[1]+self.tweet_size[1])) and spoint.y<(h-self.tweet_offset[1]):
                print "tweet box!"
                self.client.PostUpdate(self.gettext())
                self.restart()
                return True
        ...
        if self.rescalenow:
            ...
            for rectangle, name in zip(self.makerects(self.mainrect, probs),
                ...
                if vpoint.isinside(rectangle):
                    ...
                    if options.pyspell:
                        # Note that '$' is actually output as ' ' (space)
                        # But we want PySpell to interpret it as return

                        output = self.pyspell.stdin.write(name)
                        # if name==' ':
                        #     output = self.pyspell.stdin.write('return')
                        # else:
                        #     output = self.pyspell.stdin.write(name)
                        print "Wrote %s to pyspell" % name

                        # In PySpell mode
                        # after a complete word (i.e. space)
                        # pause and restart
                        if name == ' ':
                            time.sleep(5)
                            self.textbuf.set_text("")
...
#        self.sinterect = spointrect
	...
	self.pixmap.draw_rectangle(widget.get_style().black_gc, True,
                                   self.reset_offset[0], height-(self.reset_offset[1]+self.reset_size[1]), self.reset_size[0], self.reset_size[1])

        if options.twitter:
            # twitter box
            self.pixmap.draw_rectangle(widget.get_style().black_gc, True,
                                       self.tweet_offset[0], height-(self.tweet_offset[1]+self.tweet_size[1]), self.tweet_size[0], self.tweet_size[1])
...
#             print "prefix: '%s'" % prefix
        ...
        if options.ignore_vicon:
            ...
            # big mouse cursor
            self.pixmap.draw_rectangle(gc, True, spoint.x-2, spoint.y-2, 20, 20);
...
#            scale = Point(1.0, 1.0)
    ...
    def say(self, text):
        ...
        try:
            pid = Popen(["/usr/bin/say",text]).pid
        except:
            pass
        pass
# 	try:
...
# 	    pass
    ...
    def set_velocity(self):
        ...
        # Linearly map (self.zstart,maxheight) to (1.0,maxvel)
        # If z falls below zstart-zoomoutthresh then zoom out
        maxheight=1000 # mm
        maxvel = 1.15
        zoomoutthresh = 500 # in mm
        # Set velocity based on z position of object
        z = self.viconmousepos[2]

        if z>self.zstart:
            v = max(((z-self.zstart)/maxheight) * (maxvel-1.0)+1.0,1.0) #don't drop below 1
            #print "Velocity: %f" %v
            self.origvelocity=v

        th = self.zstart-zoomoutthresh
        if z<th:            
            #Compute a "negative" velocity based on z axis
            v = max( ((th-z)/maxheight) * (maxvel-1.0)+1.0,1.0) #don't drop below 1.0
            self.origvelocity=v # reset velocity to be safe
            #lower than some threshold
            self.zoomin=False # do not zoom in
            print "zoom changed False"
        else:
            self.zoomin=True

    def get_mouse_coordinates(self):
        # Convert from vicon co-ordinates to window co-ordinates

        x=((self.viconmousepos[0]-self.roomrect.x)/self.roomrect.sizex)*512.0
        y=((self.viconmousepos[1]-self.roomrect.y)/self.roomrect.sizey)*512.0
        
        return Point(x,y)
        

    def update_viconmousepos(self):
        ...
        if obj:
            ...
            if index is not None:
                t = objs[index]['t']
                if self.xr:
                    self.viconmousepos[0]=-float(t[self.idx[0]])
                else:
                    self.viconmousepos[0]=float(t[self.idx[0]])
                if self.yr:
                    self.viconmousepos[1]=-float(t[self.idx[1]])
                else:
                    self.viconmousepos[1]=float(t[self.idx[1]])
                if self.zr:
                    self.viconmousepos[2]=-float(t[self.idx[2]])
                else:
                    self.viconmousepos[2]=float(t[self.idx[2]])
    ...
    def __init__(self):
        ...
        if options.pyspell:
            self.pyspell = Popen(["python", "PySpell.py"], stdin=subprocess.PIPE,cwd="/Users/gwtaylor/python/SquidBall/games/pyspell")

        if not options.ignore_vicon:

            # order in which to read vicon (axes setup)
            self.idx = [abs(options.x)-1,abs(options.y)-1,abs(options.z)-1]
            # reverse axes?
            self.xr = options.x<0
            self.yr = options.y<0
            self.zr = options.z<0
            ...
            self.viconmousepos = [None]*3 # init

            if options.simulation_mode:
                self.f = open('vicon.txt','r')
            else:
                time.sleep(options.sleep)
                self.vp = ViconProxy()

        if options.twitter:
            self.client = twitter.Api(username="nyucrowd", password="cr0wd2cl0ud")
        ...
        width, height = (512,512) # super hacky -- how do I get this from the widget before it's drawn?

        self.reset_size=(40,40)
        self.reset_offset=(10,10) # from bottom left

        # self.tweet_offset=(self.reset_offset[0]+self.reset_size[0]+10,
        #                         self.reset_offset[1])

        if options.twitter:
            self.tweet_size=(40,40)
            self.tweet_offset=(width-10-self.tweet_size[0],self.reset_offset[1])
