...
void EventStatDialog::GetEventTypeString(EventType eventType, char* pResultString)
{
	...
	case RECOVERY_COUNT:
		strcpy( pResultString, "Recovery count" ); break;
	case SKIPPED_IMAGES:
...
void EventStatDialog::GetTimeString(TimeSliceType timeSliceType, char* pResultString)
{
    ...
    {
	case LAST_10S:
		strcpy( pResultString, "10 s" ); break;
	case LAST_30S:
		strcpy( pResultString, "30 s" ); break;
	case LAST_1MIN:
		strcpy( pResultString, "1 min" ); break;
	case LAST_5MINS:
		strcpy( pResultString, "5 mins" ); break;
	case LAST_10MINS:
		strcpy( pResultString, "10 mins" ); break;
	case LAST_15MINS:
		strcpy( pResultString, "15 mins" ); break;
	default:
...
void EventStatDialog::InitializeColumns()
{
	...
	m_timeSliceComboBox.AddString("Last 1min / 5mins / 10mins / 15mins");		
...
void EventStatDialog::UpdateColumnHeader()
{
	...
	for (int i = 0; i < sk_numOfColumns; i++)
	{
		...
		sprintf(headerBuff,"Last %s", buff);
			...
			*/
			...
			else
			{
				if (nextTimeSlice != LAST_10S)
				{
					...
					//nextTimeSlice = (TimeSliceType)((unsigned int)nextTimeSlice - 1);
					continue;
				}
				else
				{
...
void EventStatDialog::OnCbnSelchangeComboTimeslice()
{
		...
		case 0:
			...
			break;
		case 1:
			m_columnsData[0] = LAST_1MIN;
			m_columnsData[1] = LAST_5MINS;
			m_columnsData[2] = LAST_10MINS;
			m_columnsData[3] = LAST_15MINS;
			break;
		default:
			//do nothing
			break;
